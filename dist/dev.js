	...
	async init() {
		...
		this.filewatchers.push(
			watch_dir(
				...
				() => {
					try {
						const new_manifest_data = core.create_manifest_data(routes);
						core.create_app({
							bundler: this.bundler,
		...
		const compilers = await core.create_compilers(this.bundler, cwd, src, dest, false);
...
function watch_dir(
	...
	Promise.resolve(require('./CheapWatch.es.js')).then(({ default: CheapWatch }) => {
		if (closed) return;
